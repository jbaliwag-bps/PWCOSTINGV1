...
namespace PWCOSTING.DAL._100
{
    public class WIPLabPlatingDAL
    {
        ...
        public List<tbl_100_WIP_COSTING_LABOR_PLATED> GetByNo(int yearused, string itemno, string partno)
        {
            try
            {
                return db.WIPLaborPlatedList.AsNoTracking().Where(w => w.YEARUSED == yearused && w.ItemNo == itemno && w.PartNo == partno).ToList();
            }
            catch (Exception ex)
            {
                throw ex;
            ...
            using (var dbContextTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    foreach (tbl_100_WIP_COSTING_LABOR_PLATED record in records)
                    {
                        ...
                        db.WIPLaborPlatedList.Attach(existrecord);
